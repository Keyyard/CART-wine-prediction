...
class ComparisonTable:
    ...
    def create_comparison_table(y_test, y_pred):
        ...
        comparison_df = pd.DataFrame({
            ...
            'Match': y_test.apply(lambda x: "Match" if ("Good" if x >= 5 else "Bad") == ("Good" if y_pred[y_test.index.get_loc(x)] >= 5 else "Bad") else "Mismatch")
        })
        return comparison_df

    @staticmethod
    def display_comparison_table(comparison_df):
        """Display the comparison table with styling using Styler.map."""
        st.subheader("Detailed Comparison: Actual vs Predicted (Wine Quality)")
        st.write(comparison_df.style.map(
            lambda x: 'background-color: lightgreen' if x == 'Match' else 'background-color: lightcoral', subset=['Match']
        ).set_table_styles(
